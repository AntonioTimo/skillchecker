...
| Зависимость из не-реестрового источника: VCS (`git+`/`hg+`/`svn+`/`bzr+`, `github:`/bare `user/repo`), произвольный URL/tarball/wheel, non-TLS `http://`, index/source-redirect (`--extra-index-url`/`--trusted-host`/`verify_ssl=false`), отравленный `resolved` в lockfile | Обходит подпись/аудит реестра; для git/tarball исполняются build-хуки скачанного пакета на install. | HI023 (HIGH) |
