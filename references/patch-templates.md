...
## § Taint: секрет из окружения уходит в сеть (TF001 / TF002)

**Когда нужно:** значение из `os.environ` / `os.getenv` доходит до сетевого вызова
(`requests`/`httpx`/`urllib`).

- **TF001** (адрес плохой/динамический: голый/кодированный IP, punycode, известный
  exfil-хост, нелитеральный URL) — это **refuse (RED)**, не патч. Скилл не должен
  читать секрет из окружения и слать его на bare-IP, encoded/punycode-хост,
  webhook или runtime-выбранный URL. Нужна исходящая авторизация — секрет остаётся
  на server-side; вокруг exfil-канала не патчим, удаляем.
- **TF002** (адрес — хардкоженный named-хост, форма легитимного API-клиента) —
  патчим/подтверждаем:

```python
# Было — секрет уходит к третьей стороне; named-хост, но проверить владельца
token = os.environ["MY_API_KEY"]
requests.post("https://api.thirdparty.example/v1", headers={"Authorization": token})

# Стало — убедиться, что named-destination владеет этим секретом; вынести вызов
# в проверенный SDK, запинить эндпоинт, не пробрасывать секрет на чужой хост.
# Если это и есть собственный сервис креденшела — оставить, это ожидаемый HIGH
# (человек подтверждает), не CRITICAL.
```

Гард: позиция URL исключена из payload-taint, поэтому конфигурируемый эндпоинт из
env (`requests.post(os.environ["API_URL"], json=data)`) с не-секретным телом не
флагается и патча не требует.
