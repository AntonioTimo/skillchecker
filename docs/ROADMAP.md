...
| — | **Set-model unification (rounds 8–13, Codex rejects 1–3 + multi-agent re-sweeps, no new rule IDs):** the four per-scope walkers collapsed into ONE ValueFacts abstract interpreter (single binding/resolution source, −562 lines, behind a differential golden harness); a callee resolved as the SET of its possible values (`_VF.members` + positional `seq`), CLOSED under every expression constructor (Attribute/getattr/Subscript/Path-ctor/IfExp/for-target/comprehension) with a commutative join + order-independent nested-union dedup; `AST002` latent-regression fix caught by CI. Shipped via #12 | 1.11.1 |
